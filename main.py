...
from anthropic import AsyncAnthropic # استخدام النسخة غير المتزامنة
from llama_index.core import StorageContext, load_index_from_storage, Settings
from llama_index.embeddings.cohere import CohereEmbedding

# ========== KEYS ==========
TELEGRAM_TOKEN = os.environ.get("TELEGRAM_TOKEN")
print("DEBUG - Telegram token loaded:", bool(TELEGRAM_TOKEN))
...
# ========== CLAUDE CLIENT ==========
client = AsyncAnthropic() # تهيئة العميل غير المتزامن
...
async def handle_message(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        # Step 1 — search the knowledge base asychronously
        nodes = await retriever.aretrieve(user_message) # تغيير الدالة إلى aretrieve
...
Explain this to the student following the rules above."""
        ...
        # Step 3 — call Claude asynchronously with CORRECT model name
        # Step 3 — call Claude asynchronously
        message = await client.messages.create(
            model="claude-sonnet-4-6", # <-- The exact, official Sonnet model name
            max_tokens=2000,
        ...
        print(f"Error in handle_message: {e}") # لطباعة الخطأ الفعلي في الكونسول لتسهيل حله مستقبلاً

# ========== RUN BOT ==========

if __name__ == '__main__':
    if not TELEGRAM_TOKEN:
        print("CRITICAL ERROR: TELEGRAM_TOKEN is missing. Check your .env file.")
        sys.exit(1)
